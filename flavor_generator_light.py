                    ...
                    if isinstance(linter, dict) and 'linter_name' in linter:
                        ...
                        if linter_name in components:
                            if new_flavor not in linter['descriptor_flavors']:
                                ...
                                modified = True
                                logger.info(f"Added {new_flavor} to {linter_name} in {file_path}")
                                ...
                                if 'install' in data and not root_flavor_added:
                                    ...
                                    if new_flavor not in data['descriptor_flavors']:
                                        ...
                                        logger.info(f"Added {new_flavor} to root descriptor_flavors in {file_path}")
                        else:
                            if new_flavor in linter['descriptor_flavors']:
                                linter['descriptor_flavors'].remove(new_flavor)
                                modified = True
                                logger.info(f"Removed {new_flavor} from {linter_name} in {file_path}")
...
def generate_flavor_files(new_flavor: str, new_flavor_description: str) -> None:
    """Generate flavor files using generate_flavor from build.py."""
    logger.info(f"Generating flavor files for {new_flavor}")

    config.init_config("build")

    flavors = megalinter.flavor_factory.list_megalinter_flavors()

    if new_flavor not in flavors:
        flavors[new_flavor] = {"label": new_flavor_description}

    generate_flavor(new_flavor, flavors[new_flavor])

    logger.info(f"Flavor files generated for {new_flavor}")
...
def main() -> None:
    ...
    try:
        ...
        logger.info("Starting flavor files generation")
        generate_flavor_files(new_flavor, new_flavor_description)
        logger.info("Flavor files generation completed successfully")
